...
public class AgentZaOsiguranje {
	...
	static void sortiraniPrikazKategorijaZaDan(double[][] eo) {
		...
		do {			
			...
			sortiranjeNizove(sortKategorije,  Poredak.RASTUCI);
	...
	static void sortiraniDanaZaKategoriju(double[][] eo) {
		...
		do {			
			...
			sortiranjeNizove(sortDana, Poredak.RASTUCI);
	...
	public static void sortiranjeNizove(double[] nizSort, Poredak poredak) {
		int n = nizSort.length;
		boolean zameni;
		do {
			zameni = false;
			for (int i = 0; i < n - 1; i++) {
				if ((poredak == Poredak.RASTUCI && nizSort[i] > nizSort[i + 1]) || 					(poredak == Poredak.OPADAJUCI && nizSort[i] < nizSort[i + 1])) {
